...
#Region "전략 라벨 표시/색상 일원화 (Helper)"

''' <summary>
''' 전략 라벨 텍스트/색상을 일원화해서 갱신:
''' - 적용 On:  "전략 적용: {이름} [Full Chart|Simulation]" + 빨간색
''' - 적용 Off: "전략 미적용" + 회색(180,180,180)
''' - scopeLabel 미지정 시, 현재 토글/시뮬 상태로 자동 판별
''' - 차트가 SetStrategyLabelColor(Color)를 제공하면 리플렉션으로 호출(없으면 조용히 패스)
''' </summary>
Private Sub UpdateStrategyApplyBadge(Optional scopeLabel As String = Nothing)
    Try
        ...
        If isOn AndAlso Not String.IsNullOrWhiteSpace(strategyName) Then
            ...
            End If
            labelText = $"전략 적용: {strategyName} [{scopeLabel}]"
        Else
            labelText = "전략 미적용"
        End If
...
''' <summary>
''' 차트 컨트롤이 SetStrategyLabelColor(Color) API를 제공하면 색상 전달(없으면 무시).
''' </summary>
